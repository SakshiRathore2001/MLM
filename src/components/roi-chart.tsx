...
const generateSimulationData = () => {
    ...
    for (let month = 1; month <= 36; month++) {
        const capitalBeforeProfit = capital;
        ...
        data.push({
            month: `M${month}`,
            capitalBeforeProfit: Math.round(capitalBeforeProfit),
...
const chartConfig = {
    capitalBeforeProfit: {
      label: "Capital Before Profit",
      color: "hsl(var(--chart-3))",
    },
    newCapital: {
      label: "New Capital (After Reinvestment)",
        ...
        <ChartContainer config={chartConfig} className="h-80 w-full">
            <ResponsiveContainer>
                <LineChart data={simulationData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                    ...
                    <Line 
                        type="monotone" 
                        dataKey="capitalBeforeProfit" 
                        stroke="var(--color-capitalBeforeProfit)" 
                        strokeWidth={2}
                        dot={false}
                    />
                    <Line 
